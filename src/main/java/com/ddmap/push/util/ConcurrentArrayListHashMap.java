...
/**
 * @Title: ConcurrentArrayListHashMap.java
 * @Package com.ddmap.push.util
 * @Description: TODO(添加描述)
 * @author guo
 * @date 2011-11-18 下午9:17:25
 * @version V1.0
 */
public class ConcurrentArrayListHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V>, Serializable {
	...
	/**
	 * Number of updates that alter the size of the map. This is used during bulk-read methods to make sure they see a
	 ...
	 */
	private transient final AtomicInteger modCount = new AtomicInteger();

	private transient int arrayVersion;
	private transient Object[] array = new Object[0];
	 ...
	 * is critical because ConcurrentArrayListHashMap uses power-of-two length hash tables, that otherwise encounter
	...
	/**
	 * ConcurrentArrayListHashMap list entry. Note that this is never exported out as a user-visible Map.Entry.
	 * 
	...
	static final class Segment<K, V> extends ReentrantLock implements Serializable {
		 ...
		 * new nodes are created to replace them. This works well for hash tables since the bin lists tend to be short.
		 * (The average length is less than two for the default load factor threshold.) Read operations can thus proceed
		 * without locking, but rely on selected uses of volatiles to ensure that completed write operations performed
		 * by other threads are noticed. For most purposes, the "count" field, tracking the number of elements, serves
		 * as that volatile variable ensuring visibility. This is convenient because this field needs to be read in many
		 * read operations anyway: - All (unsynchronized) read operations must first read the "count" field, and should
		 * not look at table entries if it is 0. - All (synchronized) write operations should write to the "count" field
		 * after structurally changing any bin. The operations must not take any action that could even momentarily
		 * cause a concurrent read operation to see inconsistent data. This is made easier by the nature of the read
		 * operations in Map. For example, no operation can reveal that the table has grown but the threshold has not
		 * yet been updated, so there are no atomicity requirements for this with respect to reads. As a guide, all
		 * critical volatile reads and writes to the count field are marked in code comments.
		 */
		...
		ConcurrentArrayListHashMap<K, V> map;

		Segment(ConcurrentArrayListHashMap<K, V> map, int initialCapacity, float lf) {
			this.map = map;
			loadFactor = lf;
	 ...
	 */
	public ConcurrentArrayListHashMap(int initialCapacity, float loadFactor, int concurrencyLevel) {
	 ...
	 *        the map
	 */
	public ConcurrentArrayListHashMap(Map<? extends K, ? extends V> m) {
	...
	public boolean isEmpty() {
		...
		/*
		 * We keep track of per-segment modCounts to avoid ABA problems in which an element in one segment was added and
		 * in another removed during traversal, in which case the table was never actually empty at any point. Note the
		 * similar use of modCounts in the size() and containsValue() methods, which are the only other methods also
	 ...
	 * operations.
	 * 
	 * <p> The view's <tt>iterator</tt> is a "weakly consistent" iterator that will never throw
	...
	public Object[] arrayValues() {
		if (arrayVersion == modCount.get()) {
			return array;
		} else {
			Collection<V> vs = values();
			array = vs.toArray();
			arrayVersion = modCount.get();
			return array;
		}
	}
	...
	abstract class HashIterator {
		...
		public void remove() {
			...
			ConcurrentArrayListHashMap.this.remove(lastReturned.key);
			lastReturned = null;
		}
	}
	...
	final class WriteThroughEntry extends AbstractMap.SimpleEntry<K, V> {
		 ...
		 * do not and cannot guarantee more.
		 */
		public V setValue(V value) {
			if (value == null)
				throw new NullPointerException();
			V v = super.setValue(value);
			ConcurrentArrayListHashMap.this.put(getKey(), value);
			return v;
		}
	}
	...
	final class KeySet extends AbstractSet<K> {
		...
		public int size() {
			return ConcurrentArrayListHashMap.this.size();
		}

		public boolean contains(Object o) {
			return ConcurrentArrayListHashMap.this.containsKey(o);
		}

		public boolean remove(Object o) {
			return ConcurrentArrayListHashMap.this.remove(o) != null;
		}

		public void clear() {
			ConcurrentArrayListHashMap.this.clear();
		}
	}
	...
	final class Values extends AbstractCollection<V> {
		...
		public boolean contains(Object o) {
			return ConcurrentArrayListHashMap.this.containsValue(o);
		}
	...
	final class EntrySet extends AbstractSet<Map.Entry<K, V>> {
		...
		public boolean contains(Object o) {
			...
			V v = ConcurrentArrayListHashMap.this.get(e.getKey());
		...
		public boolean remove(Object o) {
			if (!(o instanceof Map.Entry))
				return false;
			Map.Entry<?, ?> e = (Map.Entry<?, ?>) o;
			return ConcurrentArrayListHashMap.this.remove(e.getKey(), e.getValue());
		}
	...
	/**
	 * Save the state of the <tt>ConcurrentArrayListHashMap</tt> instance to a stream (i.e., serialize it).
	 * 
	 * @param s
	 *        the stream
	...
	/**
	 * Reconstitute the <tt>ConcurrentArrayListHashMap</tt> instance from a stream (i.e., deserialize it).
	 * 
	 * @param s
	 *        the stream
	 */
